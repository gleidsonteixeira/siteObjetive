...
//GLOBAL VARIABLES
    ...
    //                 chamadosEmAberto++;
    //             }
    //             var html =  '<a href="admin-ticket.php?chamado_id='+c.val().chamado_id+'">'+
    //                             '<h6 class="codigo">'+
    ...
    // function listaChamadosAnalista(analista_id){
    //     var database = firebase.database().ref();
    //     var databaseRef = database.child('chamados').orderByChild('analista_id').equalTo(analista_id);
    //     var listaChamados = document.getElementById('listaChamados');
    //     var chamadosEmAberto;
    ...
    //                 chamadosEmAberto++;
    //             }
    //             var html = '<a href="chamado.php?chamado_id='+c.val().chamado_id+'">'+
    //                         '<h5 class="mini-title">'+
    //                             '<b class="light-blue white-text z-depth-2">#'+c.val().chamado_indice+'</b>'+
    //                             '<b class="tipo light-blue white-text z-depth-2">'+c.val().tipo+'</b>'+
    //                             '<span class="right">'+c.val().data_solicitacao+'</span>'+
    //                         '</h5>'+
    //                         '<h4 class="light truncate">'+c.val().titulo+'</h4>'+
    //                         '<div class="titulo">'+
    //                             '<div class="foto circle z-depth-2">'+
    //                                 '<img src="'+c.val().usuario_foto+'" class="p_foto responsive-img" />'+
    //                                 '<div class="legenda status'+c.val().status+'" data-status="'+c.val().status+'"></div>'+
    //                             '</div>'+
    //                             '<h6>'+
    //                                 '<b class="truncate">'+c.val().usuario_nome+'</b>'+
    //                                 '<span class="">'+c.val().usuario_empresa_nome+'</span>'+
    //                             '</h6>'+
    //                             '<h6 class="nm">'+
    //                                 '<b class="light-blue-text">'+c.val().usuario_email+'</b>'+
    //                             '</h6>'+
    //                         '</div>'+
    //                         '<div class="mini-descricao">'+c.val().descricao+'</div>'+
    //                             '<p>'+
    //                                 '<b>Prioridade:</b>'+
    //                                 '<span class="white-text prioridade prioridade'+c.val().prioridade+'">'+
    //                                     '<b>'+prioridade+'</b>'+
    //                                 '</span>'+
    //                             '</p>'+
    ...
    //                                     '<span class="circle">'+c.val().anexos+'</span>'+
    //                                 '</button>'+
    //                                 '<button disabled class="circle">'+
    //                                     '<i class="mdi-communication-chat"></i>'+
    //                                     '<span class="circle">'+c.val().respostas+'</span>'+
    //                                 '</button>'+
    //                         '</div>'+
    //                     '</a>';
    ...
    //     });
    //     setTimeout(function(){
    //         databaseRef2 = database.child('chamados').orderByChild('quem_abriu_chamado').equalTo(analista_id);
    ...
    function listaChamadosCliente(){
        var usuario_id = $(".u_id").text();
        var databaseRef = database.child('chamados/'+usuario_id);
        var listaChamados = document.getElementById('listaChamados');
        var chamadosEmAberto;
        ...
        databaseRef.on('value', function(snapshot){
            ...
            snapshot.forEach( c => {
                ...
                var html =  '<a href="ticket.php?chamado_id='+c.val().chamado_id+'='+c.val().quem_abriu_chamado+'">'+
                                '<h6 class="codigo">'+
    ...
    function novoChamado(){
        ...
        var chamadoRef = database.child("chamados/"+$(".u_id").text());
        ...
        $("#novoChamado .form button").click(function(e){
            ...
            if(tituloChamado == ''){
                ...
                alert("Selecione quem solicita o ticket");
            }else{            
                novoChamado.set({
                ...
                $("#novoChamado").removeClass("active");
                $("#alertaT p").text("Ticket criado com sucesso!")
    ...
    //         var chamadoRef = database.child("chamados/"+chamado_id);
    //         var chamadoRef2 = database.child("chamados");
    ...
    //         $("#alertaT p").text("Deseja realmente apagar esse chamado?");
    ...
    //             $("#alertaT p").text("Chamado apagado com sucesso!");
    //             setTimeout( function(){
    //                 $("#alertaT").removeClass("active");
    //             }, 2000 );
    //             setTimeout(function(){
    ...
    //             },1000);
    //         });
    //         chamadoRef2.on('child_removed', function(snap){
    //             var id = $(".u_id").val();
    //             var nivel = $(".u_nivel").text();
    //             if(nivel == 'admin'){
    //                 listaChamados();
    ...
    function ResponderChamado(){
        ...
        $("#responderChamado").submit(function(e){
            ...
            var analista_foto = $(".u_foto").attr("src");
            var analista_email = $(".u_email").val();
        ...
        statusRef.on('child_changed',function(data){
            if(data.val() == 6){
                ...
                if(nivel == 'cliente'){
                    ...
                    //     var html =  '<span class="mini-title white-text">'+
                    //                     '<b>Seu chamado foi solucionado ?:</b>'+
                    //                 '</span>'+
                    //                 '<br>'+
                ...
                if(nivel == 'cliente'){
                    ...
                    aviso.innerHTML = html;
                    listaResposta.appendChild(aviso);
                    $("#alertaT p").text("O cliente encerrou o ticket.");
                    $(".chamado-item.aviso button").slideUp("fast");
                    chamaAlerta();
                }
            }else if(data.val() == 2){
    ...
    //             $("#alertaT p").text("Selecione um analista primeiro");
    //             chamaAlerta();
    //         }else{
    //             chamadoRef = database.child('chamados/'+chamado_id);
    //             chamadoRef.update({
    //                 analista_id:analistaSelecionado
    //             });
    //             $("#analistas").removeClass("active");
    //             $("#alertaT p").text("Analista adicionado com sucesso!");
    //             chamaAlerta();
    //         }
    //     });
    // }adicionarAnalista();
    ...
    //     $('#novoUsuario form').submit(function(e){
    //         e.preventDefault();
    //         var nome = $('#nome').val();
    //         var email = $('#email').val();
    //         var empresaId = $('#n-empresa').attr('data-empresa');
    //         var empresaNome = $('#n-empresa').val();
    //         var nivel = $('#n-nivel option:selected').val();
    //         var password = 'abcd1234';
    //         if(empresaNome == '' || empresaNome == null){
    //             alert('Escolha uma empresa para o usuário');
    //         }
    //         if(nivel == '#'){
    //             alert('Escolha um nível para o usuário');
    //         }
    ...
    //                 // Handle Errors here.
    //                 setTimeout(function(){
    //                     var usuarioRef = database.child('empresas_usuarios/' + user.uid);
    //                     if(user != null){
    //                         usuarioRef.set({
    //                             ativo: 1,
    ...
    //                             tipo : nivel,
    //                             usuario : email,
    //                             usuario_foto : 'https://firebasestorage.googleapis.com/v0/b/portaldechamados.appspot.com/o/usuarios%2Fp1.png?alt=media&token=8e02e7d6-724a-4cab-bbc6-4e14f040784a',
    //                             usuario_id : user.uid
    //                         });            
    //                     }
    //                 }, 2000);
    //                 $('#novoUsuario').removeClass('active');
    //                 $('#alert p').text('Usuário criado com sucesso!');
    //                 chamaAlerta();
    ...
    function carregarUsuario(){
        var urlRef = window.location.search.slice(1).split("=");
        var usuarioId = urlRef[1];
        var usuarioNome = $(".perfil .nome").text();
        var usuarioCargo = $(".perfil .cargo").text();
        var usuariosRef = database.child("tickets-usuarios/"+usuarioId);
        var usuarioFoto = $(".perfil img").attr("src");
        $(".u_id").val(usuarioId);
        $("#contaChamado").val(usuarioId);
